...
func TestPriorityQueue_Shrink(t *testing.T) {
	var compare tile.Comparator[int] = func(a, b int) int {
		if a < b {
			return -1
		}
		if a == b {
...
func priorityQueueOf(capacity int, data []int, compare tile.Comparator[int]) *PriorityQueue[int] {
...
func compare() tile.Comparator[int] {
